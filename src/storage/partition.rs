...
impl <'a> Partition<'a> {
    pub fn new(block_device: &'a BlockDevice, partition: Vec<u8>) ->Partition<'a> {
        ...
        let start_sector_lba = encoded_chs_to_lba(partition[CHS_FIRST_SECTOR_OFFSET], partition[CHS_FIRST_SECTOR_OFFSET + 1], partition[CHS_FIRST_SECTOR_OFFSET + 2]);
        let last_sector_lba = encoded_chs_to_lba(partition[CHS_LAST_SECTOR_OFFSET], partition[CHS_LAST_SECTOR_OFFSET + 1], partition[CHS_LAST_SECTOR_OFFSET + 2]);
...
impl <'a> BlockDevice for Partition<'a> {
    fn read_blocks(&self, offset: usize, number: usize) ->Vec<u8> {
        self.block_device.read_blocks(self.start_sector_lba + offset, number)
    }
